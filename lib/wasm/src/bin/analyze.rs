...
use wasm::{wimpl::{FunctionId, Module, analyze::{collect_call_indirect_idx_expr, print_map_count, collect_i32_load_store_arg_expr, collect_memory_functions, collect_function_direct_call_count, param_exprs}, callgraph::{self, Options}}};
...
fn main() {
    ...
    let callgraph = callgraph::reachable_callgraph(&wimpl, exported_funcs.clone(), options).unwrap().callgraph;
