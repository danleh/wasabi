...
use self::{LocalOp::*, GlobalOp::*, LoadOp::*, StoreOp::*};
...
impl LoadOp {
    fn to_type(&self) -> InstrType {
        match *self {
            I32Load => InstrType::new(vec![], vec![I32]),
            I64Load => InstrType::new(vec![], vec![I64]),
            F32Load => InstrType::new(vec![], vec![F32]),
            F64Load => InstrType::new(vec![], vec![F64]),

            I32Load8S => InstrType::new(vec![], vec![I32]),
            I32Load8U => InstrType::new(vec![], vec![I32]),
            I32Load16S => InstrType::new(vec![], vec![I32]),
            I32Load16U => InstrType::new(vec![], vec![I32]),
            I64Load8S => InstrType::new(vec![], vec![I64]),
            I64Load8U => InstrType::new(vec![], vec![I64]),
            I64Load16S => InstrType::new(vec![], vec![I64]),
            I64Load16U => InstrType::new(vec![], vec![I64]),
            I64Load32S => InstrType::new(vec![], vec![I64]),
            I64Load32U => InstrType::new(vec![], vec![I64]),
        }
    }
}
...
impl StoreOp {
    fn to_type(&self) -> InstrType {
        match *self {
            I32Store => InstrType::new(vec![I32], vec![]),
            I64Store => InstrType::new(vec![I64], vec![]),
            F32Store => InstrType::new(vec![F32], vec![]),
            F64Store => InstrType::new(vec![F64], vec![]),

            I32Store8 => InstrType::new(vec![I32], vec![]),
            I32Store16 => InstrType::new(vec![I32], vec![]),
            I64Store8 => InstrType::new(vec![I64], vec![]),
            I64Store16 => InstrType::new(vec![I64], vec![]),
            I64Store32 => InstrType::new(vec![I64], vec![]),
        }
    }
}
