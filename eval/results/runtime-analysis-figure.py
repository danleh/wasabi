...
df["hooks"] = df["instrumented hooks"]
print df

# drop some columns and rows that add clutter
df = df.drop(["comment", "analysis"], axis=1)
...
for i, row in df.iterrows():
	none_seconds = df[(df.hooks == "none") & (df.program == row.program)].iloc[0].seconds
...
g = sns.factorplot(x="hooks", y="overhead", hue="program", kind="bar", aspect=4, size=4, data=df, legend=False, estimator=np.median,errwidth=.5,capsize=.1)
